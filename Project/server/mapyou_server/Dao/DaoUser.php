...
class DaoUser implements DaoInterfaceUser {
   ...
   public function getUserByNickname($nickname){
    ...
    public function deleteAccount($nickname){
		...
		try {
			$link->beginTransaction();
			$count =  $link->exec("delete from user where nickname='".$nickname."';");
			$link->commit();
			return $count;
		} catch (Exception $e) {
			$link->rollBack();
	...
	public function isRegistredFacebook(User $user,$idNotification){
		...
		try {
			 $link->beginTransaction();
             $count=$link->exec("update user set idNotification='".$idNotification."' where email='".$user->getEmail()."' and password='".$user->getPassword()."';");
             
             if($count==0){
             	$queryReg="INSERT INTO user(nickname, password, email,idNotification) VALUES ('".$user->getNickname()."','".$user->getPassword()."','".$user->getEmail()."','".$idNotification."');";
                ...
                if($count2==1){
                	...
                	$link->commit();
                	return  $this->getUserByEmailAndPassword($user);
                }
     ...
     public function getUserByEmailAndPassword($user){
		global $link;
		$row=$link->query(
				"select * from user where email='".$user->getEmail()."' and password='".$user->getPassword()."';")
				->fetch();
		if($row){
			$u = new User();
			$u->setEmail($row['email']);
