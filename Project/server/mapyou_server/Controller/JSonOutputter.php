...
class JSonOutputter {
    ...
    public function notificationToJson($not){
		$results = array();
		if(is_array($not)){
		...
		}else if($not instanceof Notification){
			$results[] = array('id'=>$not->getModelID(),
					'notifier'=>$this->userToJson($not->getNotifier()),
                    'notified'=>$this->userToJson($not->getNotified()),
                    'mapme'=>$this->mapmeToJson($not->getMapme()),
                    'state'=>$not->getState(),
                    'date'=>$notification->getDate(),
					'type'=>$not->getType());
		}
